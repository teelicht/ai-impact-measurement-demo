...
function trend(monthly: OperationalMonth[], field: 'approvedParents' | 'tokens' | 'toolSpend',
  ...
  periods: OperationalPeriod[], synthetic: boolean, scale = 1): string {
  ...
  const rows = monthly.map((row, index) => `<tr>${headingCell(`${row.label} (${row.month})`)}` +
    `${cell(measure(values[index], row.measureStatus[coverage]))}</tr>`).join('');
...
export function renderHtml(view: ReportView): string {
  ...
  const impactRows = before && after ? comparison('Approved parent Stories / Tasks', 'approvedParents', before, after) +
...
<title>${text(report.scope)} | AI-assisted development evidence</title>
<style>
...
.ledger-control{display:flex;gap:12px;align-items:center;margin:14px 0;flex-wrap:wrap}.ledger-control select{font:inherit;border:1px solid var(--border);padding:6px 10px;background:var(--panel);color:var(--text)}#ledger table{min-width:880px}.separate{border-top:2px solid var(--border);padding-top:28px}.financials{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1px;background:var(--border);border:1px solid var(--border);margin:20px 0}.financials div{background:var(--panel);padding:16px}.financials strong{font-size:1.2rem;font-variant-numeric:tabular-nums;display:block}.financials span{color:var(--muted);font-size:12px}footer{padding:24px max(24px,calc((100vw - 1320px)/2)) 60px;color:var(--muted);font-size:12px;border-top:1px solid var(--border)}
@media(max-width:800px){header{padding:16px 18px}h1{font-size:23px;margin:4px 0}header .sub{font-size:12px}.ai-banner{margin-top:8px;padding:6px 10px;font-size:12px}main.shell{padding:14px 14px 56px}.top .shell{padding:6px 14px}.top .brand{display:none}.top nav{width:100%;gap:4px 14px;flex-wrap:nowrap;overflow-x:auto;white-space:nowrap}.top nav a{padding:3px 0;flex:none}.chart-grid,.split,.details-grid,.context-grid,.profile-grid{grid-template-columns:1fr}.context-grid{gap:10px}.context-panel{padding:12px 14px}.financials{grid-template-columns:repeat(2,minmax(0,1fr))}.type-chart{height:260px}}
...
<header><div class="eyebrow">AI-assisted development | Output report</div><h1>${text(report.scope)} engineering output report</h1><p class="sub">${report.sourceKind === 'synthetic' ? 'Synthetic example, not a live team result' : 'Configured sources (live status unverified)'} · ${text(period)} · ${text(report.timezone)} · extracted ${text(report.extractedAt)}</p><div class="ai-banner">${report.sourceKind === 'synthetic' ? `More systematic AI use from <b>2026-03</b> (M7). M1-M6 already includes AI use; this is an observed period split, not an AI-free baseline.` : before && after ? `Descriptive comparison: ${text(before.label)} (${text(before.startMonth)} to ${text(before.endMonth)}) versus ${text(after.label)} (${text(after.startMonth)} to ${text(after.endMonth)}). No AI adoption event is documented for these sources.` : 'Comparison unavailable for fewer than two months. No AI adoption event is documented for these sources.'}</div></header>
<div class="top"><div class="shell"><span class="brand">AI / delivery evidence</span><nav aria-label="Report sections"><a href="#overview">Overview</a><a href="#profile">Report profile</a><a href="#issue-volume">Issue types</a><a href="#utilization">Utilization</a><a href="#impact">Impact</a><a href="#git">Git</a><a href="#cost">Cost</a><a href="#ledger">Ledger</a><a href="#evidence">Evidence</a><a href="#migration">Migration case</a></nav></div></div>
<main class="shell">
<section id="overview"><h2>Overview</h2><div class="context-grid"><div class="context-panel"><h3>Team and service</h3><p>${supplied(context?.description)}</p></div><div class="context-panel"><h3>AI use and context</h3><p>${supplied(context?.aiUse)}</p><p class="context-secondary">${supplied(context?.concurrentChanges)}</p></div><div class="context-panel"><h3>Repository evidence</h3><div class="repo-chips">${repositoryLabels}</div></div></div><div class="kpis"><div class="kpi"><div class="kpi-label">Months covered</div><div class="kpi-value">${number(report.monthly.length)}</div><div class="kpi-meta">${text(period)}</div></div><div class="kpi"><div class="kpi-label">Approved parents</div><div class="kpi-value">${total('approvedParents')}</div><div class="kpi-meta">first release approval</div></div><div class="kpi"><div class="kpi-label">Git commits</div><div class="kpi-value">${total('commits')}</div><div class="kpi-meta">eligible, non-merge</div></div><div class="kpi"><div class="kpi-label">Token use</div><div class="kpi-value">${total('tokens')}</div><div class="kpi-meta">input + output tokens</div></div><div class="kpi"><div class="kpi-label">Recorded tool spend</div><div class="kpi-value">${recordedSpend === 'Unavailable' ? recordedSpend : `${recordedSpend} ${text(report.currency)}`}</div><div class="kpi-meta">not total AI cost</div></div>${report.sourceKind === 'synthetic' && splitMonth ? `<div class="kpi"><div class="kpi-label">Increased AI use from</div><div class="kpi-value">${text(splitMonth)}</div><div class="kpi-meta">synthetic M7 boundary</div></div>` : ''}</div></section>
<section id="profile"><h2>Report profile</h2><p class="profile-updated">Report updated ${supplied(context?.updatedAt)} · Data extracted ${text(report.extractedAt.slice(0, 10))}</p><dl class="profile-grid"><div class="profile-field"><dt>Owner and description</dt><dd>${supplied(context?.owner)}. ${supplied(context?.description)}</dd></div><div class="profile-field"><dt>Scope and use</dt><dd>${supplied(context?.includedWork)} Exclusions: ${supplied(context?.excludedWork)}</dd></div><div class="profile-field"><dt>Actual use and context</dt><dd>${supplied(context?.aiUse)} ${supplied(context?.concurrentChanges)}</dd></div><div class="profile-field"><dt>Question and comparison</dt><dd>${supplied(context?.question)} ${text(period)}. ${before && after ? `${text(before.label)} versus ${text(after.label)}.` : 'Comparison unavailable.'}</dd></div><div class="profile-field"><dt>Measures and findings</dt><dd>Approved parents: ${total('approvedParents')}; incoming production bugs: ${total('incomingBugs')}. Delivery time and human effort: ${report.leadTime === null || report.humanEffort === null ? 'unassessed' : 'available'}.</dd></div><div class="profile-field"><dt>Resources and financial claims</dt><dd>Recorded tool spend: ${recordedSpend === 'Unavailable' ? recordedSpend : `${recordedSpend} ${text(report.currency)}`}. Total AI cost and API-team ROI: unassessed. ${supplied(context?.resourceLimits)} Controls: ${supplied(context?.controls)}</dd></div></dl></section>
...
<section id="utilization"><div class="section-head"><h2>Utilization</h2><p>Created issues and token consumption, by month. Each chart has its own scale; sharing a month does not link tokens to a task or establish an AI effect.</p></div><div class="chart-grid">${issueTrend(report.monthly,null,report.periods,report.sourceKind === 'synthetic','Created issues')}${trend(report.monthly,'tokens','Input + output tokens','millions','tokens',report.periods,report.sourceKind === 'synthetic',1_000_000)}</div>
...
<h3>Usage composition</h3>${hasDetailedUsage
...
<div class="details-grid"><div><h3>Counting choices</h3><p class="mini">Children excluded: ${number(report.exclusions.children)} · emergency fixes: ${number(report.exclusions.emergency)} · duplicate approvals: ${number(report.exclusions.duplicateApprovals)} · merges: ${number(report.exclusions.mergeCommits)} · bots: ${number(report.exclusions.botCommits)} · ambiguous commit links: ${number(report.exclusions.ambiguousCommitLinks)} · unlinked commits: ${number(report.exclusions.unlinkedCommitLinks)}</p></div><div><h3>Work in progress and missing timestamps</h3><p class="mini">Open parents: ${periodTotal('openParents')} · missing start times: ${periodTotal('missingStartTimes')} · unknown production bugs: ${periodTotal('unknownProductionBugs')}. Lead time: ${number(report.leadTime)}. Human effort: ${report.humanEffort === null ? 'unassessed' : number(report.humanEffort)}.</p></div></div></section>
...
<section id="cost"><div class="section-head"><h2>Cost</h2><p>Recorded ${text(report.currency)} tool spend is not total AI cost.</p></div><div class="chart-grid">${trend(report.monthly,'toolSpend','Recorded tool charges',report.currency,'toolSpend',report.periods,report.sourceKind === 'synthetic')}<div class="chart-card">${periodTable(costRows,'Recorded consumption, subscriptions and other charges',report.periods)}</div></div><p class="cost-note">Total AI cost: unassessed. API-team ROI: unassessed. Human review, correction, enablement, governance and platform costs are unpriced; tool spend per approved parent is not a return on investment.</p><div class="table-scroll"><table><caption>Monthly billing mix and spend per approved parent</caption><thead><tr><th scope="col">Month</th><th scope="col">Charge categories</th><th scope="col">Spend / approved parent</th></tr></thead><tbody>${report.monthly.map(row => `<tr>${headingCell(row.label)}${cell(row.billingCategories === null ? 'Unavailable' : Object.entries(row.billingCategories).map(([category, amount]) => `${text(category)}: ${money(amount, report.currency)}`).join('; ') || 'No recorded charges')}${cell(row.toolSpendPerApprovedParent === null ? 'Unavailable' : money(row.toolSpendPerApprovedParent, report.currency))}</tr>`).join('')}</tbody></table></div></section>
...
<section id="evidence"><h2>Evidence</h2><p>Scope: ${text(report.scope)}. Selection: first qualifying release approval per eligible parent in complete monthly windows ${text(period)}. ${report.sourceKind === 'synthetic' ? 'M7 marks more systematic AI use; M1-M6 already includes AI use, so it is not an AI-free baseline.' : 'Configured period boundaries describe time only; no AI adoption event is documented.'}</p><p>Source method: ${text(report.completenessAttestation)}</p><p>Extraction: ${text(report.extractedAt)}; calendar timezone: ${text(report.timezone)}. Omitted historical effort, start events and release-linked defect follow-up are not treated as zero.</p>
...
function drawTrend(container){
  ...
  function redraw(){drawTypeChart();for(const container of document.querySelectorAll('.chart[data-field],.chart[data-type],.chart[data-total-issues]')){container.replaceChildren();drawTrend(container)}}
