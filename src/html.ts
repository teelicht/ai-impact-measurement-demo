...
const number = (value: number | null): string => (value === null ? "Unavailable" : new Intl.NumberFormat("en-US", { maximumFractionDigits: 1 }).format(value));
...
const measure = (value: number | null, coverage: Coverage["status"], format = number): string =>
	coverage === "unavailable" || value === null ? "Unavailable" : `${format(value)}${coverage === "partial" ? " (partial)" : ""}`;
const issueCount = (types: Record<string, number> | null, kind: string): number | null => (types === null ? null : Object.hasOwn(types, kind) ? types[kind] : 0);

function comparison(
	label: string,
	key: keyof Pick<OperationalPeriod, "approvedParents" | "incomingBugs" | "commits" | "testTouchCommits" | "testTouchShare" | "tokens" | "toolSpend">,
	before: OperationalPeriod,
	after: OperationalPeriod,
	format = number,
): string {
	const coverageKey = key === "testTouchShare" ? "testTouchShare" : key === "testTouchCommits" ? "testTouchCommits" : key;
	return (
		`<tr>${headingCell(label)}${cell(measure(before[key], before.measureStatus[coverageKey], format))}` +
...
function periodTable(rows: string, caption: string, periods: OperationalPeriod[]): string {
	...
	const months = (period: OperationalPeriod) =>
		(Number(period.endMonth.slice(0, 4)) - Number(period.startMonth.slice(0, 4))) * 12 + Number(period.endMonth.slice(5, 7)) - Number(period.startMonth.slice(5, 7)) + 1;
	const unequal = months(periods[0]) !== months(periods[1]);
	const label = (period: OperationalPeriod) => (unequal ? `${period.label} (${months(period)} months)` : period.label);
	return (
		`<div class="table-scroll"><table><caption>${text(caption)}</caption><thead><tr>` +
...
function trend(
	...
	field: "approvedParents" | "tokens" | "toolSpend",
	label: string,
	unit: string,
	coverage: "approvedParents" | "tokens" | "toolSpend",
	...
	const rows = monthly
		.map(
			(row, index) =>
				`<tr>${headingCell(`${row.label} (${row.month})`)}` +
				`${cell(measure(values[index], row.measureStatus[coverage], (value) => (field === "toolSpend" ? money(value, unit) : number(value))))}</tr>`,
		)
		.join("");
	return (
		`<figure class="chart-card"><figcaption class="chart-head"><span class="chart-title">${text(label)}</span>` +
...
function gitTrend(monthly: OperationalMonth[], field: "commits" | "testTouchCommits", label: string, periods: OperationalPeriod[], synthetic: boolean): string {
	const rows = monthly.map((row) => `<tr>${headingCell(`${row.label} (${row.month})`)}` + `${cell(measure(row[field], row.measureStatus[field]))}</tr>`).join("");
	return (
		`<figure class="chart-card"><figcaption class="chart-head"><span class="chart-title">${text(label)}</span>` +
...
function issueTrend(monthly: OperationalMonth[], kind: string | null, periods: OperationalPeriod[], synthetic: boolean, displayLabel?: string): string {
	...
	const rows = monthly
		.map(
			(row) =>
				...
				`${cell(
					measure(
						row.issueTypes === null ? null : total ? Object.values(row.issueTypes).reduce((sum, value) => sum + value, 0) : issueCount(row.issueTypes, kind),
						row.measureStatus.issueTypes,
					),
				)}</tr>`,
		)
		.join("");
	return (
		...
		`<div class="chart" ${total ? 'data-total-issues="true"' : `data-type="${text(kind)}"`}${displayLabel ? ` data-chart-label="${text(label)}" data-unit="issues"` : ""}></div>` +
		`<div class="legend"><span><i class="raw"></i>monthly</span>` +
		(periods.length === 2
			? `<span><i class="prior-avg"></i>${text(periods[0].label)} avg</span>` +
				`<span><i class="recent-avg"></i>${text(periods[1].label)} avg</span>` +
...
function tokenIssueComparison(monthly: OperationalMonth[], periods: OperationalPeriod[], synthetic: boolean): string {
	const issues = (row: OperationalMonth): number | null =>
		row.issueTypes === null || row.measureStatus.issueTypes === "unavailable" ? null : Object.values(row.issueTypes).reduce((sum, value) => sum + value, 0);
	const base = monthly.find((row) => row.tokens !== null && row.tokens > 0 && row.measureStatus.tokens !== "unavailable" && (issues(row) ?? 0) > 0);
	const baseTokens = base?.tokens ?? null;
	const baseIssues = base ? issues(base) : null;
	const indexed = (value: number | null, baseline: number | null): number | null => (value === null || baseline === null ? null : (value / baseline) * 100);
	const rows = monthly
		.map((row) => {
		...
		.join("");
	const baselineLabel = base ? `indexed to ${base.label} (${base.month}) = 100` : "index unavailable: no month has positive values for both measures";
	return (
		`<figure class="chart-card"><figcaption class="chart-head"><span class="chart-title">Token use vs created issues</span>` +
...
function ledgerRow(row: OperationalMonth, currency: string, splitIndex: number | null): string {
	...
	return (
		`<tr data-period="${splitIndex === null ? "all" : Number(row.label.slice(1)) <= splitIndex ? "before" : "after"}">` +
...
export function renderHtml(view: ReportView): string {
	...
	const usageRows =
		before && after
			? comparison("Input + output tokens", "tokens", before, after) + comparison("Test-touch share of eligible commits", "testTouchShare", before, after, percent)
			: "";
	const impactRows =
		before && after
			? comparison("Approved Stories and Tasks", "approvedParents", before, after) +
				comparison("Incoming production bugs", "incomingBugs", before, after) +
				comparison("Eligible commits (activity)", "commits", before, after)
			: "";
	const issueTypes = [...new Set([...Object.keys(before?.issueTypes ?? {}), ...Object.keys(after?.issueTypes ?? {})])].sort();
	const issueRows =
		before && after
			? issueTypes
					.map(
						(kind) =>
							...
							`${cell(measure(issueCount(after.issueTypes, kind), after.measureStatus.issueTypes))}</tr>`,
					)
					.join("")
			: "";
	...
	const diagnosticCells = diagnosticRows.map(([label, value, format]) => `<tr>${headingCell(label)}${cell(format(value))}</tr>`).join("");
	const costRows = before && after ? comparison("Recorded tool charges", "toolSpend", before, after, (value) => money(value, report.currency)) : "";
	const periodTotal = (key: "openParents" | "missingStartTimes" | "unknownProductionBugs"): string =>
		report.monthly.some((row) => row[key] === null || row.measureStatus[key] === "unavailable")
			? "Unavailable"
			: `${number(report.monthly.reduce((sum, row) => sum + (row[key] ?? 0), 0))}${report.monthly.some((row) => row.measureStatus[key] === "partial") ? " (partial)" : ""}`;
	...
	const modelRows = Object.entries(
		...
		.map(
			([name, item]) =>
				`<tr>${headingCell(name)}${cell(number(item.inputTokens))}` +
				`${cell(number(item.cachedInputTokens))}${cell(number(item.outputTokens))}${cell(number(item.tokens))}` +
				`${cell(number(item.failed))}${cell(number(item.retries))}</tr>`,
		)
		.join("");
	const usageComposition =
		...
		(hasDetailedUsage
			...
			: `<p class="note">Model, input/output/cache, retry and work-item breakdown unavailable. ${
					report.monthly.some((row) => row.tokens !== null) ? "Only monthly token totals are recorded." : "Monthly token totals are unavailable."
				}</p>`);
	const sourceRows = report.coverage
		.map(
			(entry) =>
				`<tr>${headingCell(entry.source)}${cell(status(entry.status))}` +
				`${cell(number(entry.eligible))}${cell(number(entry.extracted))}${cell(number(entry.linked))}` +
				`${cell(number(entry.excluded))}${cell(number(entry.missing))}${cell(text(entry.reason || "No reported gap"))}</tr>`,
		)
		.join("");
	const definitions = report.definitions
		.map((entry) => `<tr>${headingCell(entry.metric)}` + `${cell(text(entry.definition))}${cell(text(entry.source))}${cell(status(entry.coverage))}</tr>`)
		.join("");
	const gapRows = report.evidenceGaps.map((gap) => `<tr>${headingCell(gap.metric)}${cell(text(gap.reason))}</tr>`).join("");
	const examples = view.examples.map((item) => `<tr>${headingCell(item.id)}${cell(text(item.kind))}` + `${cell(text(item.treatment))}</tr>`).join("");
	...
	const typeData = report.monthly
		.map(
			(row) =>
				`<tr>${headingCell(`${row.label} (${row.month})`)}${types.map((kind) => cell(measure(issueCount(row.issueTypes, kind), row.measureStatus.issueTypes))).join("")}</tr>`,
		)
		.join("");
	const embedded = JSON.stringify(view)
		.replaceAll("&", "\\u0026")
		.replaceAll("<", "\\u003c")
		.replaceAll(">", "\\u003e")
		.replaceAll("\u2028", "\\u2028")
