...
<section id="evidence"><h2>Evidence</h2><p>Scope: ${text(report.scope)}. Selection: first qualifying release approval per eligible Story or Task in complete monthly windows ${text(period)}. ${report.sourceKind === 'synthetic' ? 'M7 marks more systematic AI use; M1-M6 already includes AI use, so it is not an AI-free baseline.' : 'Configured period boundaries describe time only; no AI adoption event is documented.'}</p><p>Source method: ${text(report.completenessAttestation)}</p><p>Extraction: ${text(report.extractedAt)}; calendar timezone: ${text(report.timezone)}. Omitted historical effort, start events and release-linked defect follow-up are not treated as zero.</p>
