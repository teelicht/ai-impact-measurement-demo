...
export async function loadTicketFile(path: string): Promise<Ticket[]> {
  ...
  return tickets.map(value => {
    ...
    return {
      id, type: string(item.type, `${label}: type`), status,
      createdAt,
      ...
      approvals, provenance: { source: 'file:tickets', recordId: id },
    };
  });
}
  ...
  return usage.map(value => {
    ...
    if (item.kind === 'monthly-total') {
      if (['model', 'at', 'inputTokens', 'outputTokens', 'cachedInputTokens', 'attempt', 'outcome', 'ticketId']
        .some(field => field in item)) throw new Error(`${label}: monthly totals cannot contain event detail`);
      return { kind: 'monthly-total', id, provenance: { source: 'file:usage', recordId: id },
        month: string(item.month, `${label}: month`), tokens: number(item.tokens, `${label}: tokens`),
      };
    }
    ...
    return {
      id, provenance: { source: 'file:usage', recordId: id },
      at: string(item.at, `${label}: at`), model: string(item.model, `${label}: model`),
      ...
      attempt: number(item.attempt, `${label}: attempt`), outcome,
...
export async function loadChargeFile(path: string): Promise<Charge[]> {
  ...
  return charges.map(value => {
    ...
    return {
      id, provenance: { source: 'file:charges', recordId: id },
      billId: string(item.billId, `${label}: billId`), billTotal: number(item.billTotal, `${label}: billTotal`),
      allocatedTo: string(item.allocatedTo, `${label}: allocatedTo`),
      month: string(item.month, `${label}: month`), currency: string(item.currency, `${label}: currency`),
      category: string(item.category, `${label}: category`), amount: number(item.amount, `${label}: amount`),
