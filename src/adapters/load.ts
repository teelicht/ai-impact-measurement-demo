...
type Config = Pick<SourceBundle, "scope" | "startDate" | "endDate" | "timezone" | "extractedAt" | "currency" | "completenessAttestation"> & {
	version: 1;
...
function parseConfig(value: unknown, path: string): Config {
	...
	for (const [name, value] of Object.entries(sources)) {
		...
		if ((selector.kind === "file" || selector.kind === "module") && (typeof selector.path !== "string" || selector.path.trim() === "")) {
		...
		if (selector.kind === "git") {
			if (name !== "commits" || typeof selector.path !== "string" || !selector.path.trim() || typeof selector.revision !== "string" || !selector.revision.trim()) {
			...
			if (
				selector.botAuthors !== undefined &&
				(!Array.isArray(selector.botAuthors) || !selector.botAuthors.every((author: unknown) => typeof author === "string" && author.trim()))
			) {
...
export async function loadConfigured(configPath: string): Promise<SourceBundle> {
	...
	for (const name of names) {
		...
		if (!selector) {
			coverage.push({ source: name, status: "unavailable", eligible: 0, extracted: 0, linked: 0, excluded: 0, missing: 0, reason: "No source configured." });
			continue;
		}
		...
		try {
			if (selector.kind === "module" && sourcePath) {
				...
				const loaded = loadLocalGit(sourcePath, selector.revision, config.startDate, config.endDate, config.timezone, selector.ticketPattern, selector.botAuthors);
				result = { commits: loaded.commits, coverage: [{ ...loaded.coverage, reason: `${loaded.coverage.reason} Git extracted at ${loaded.extractedAt}.` }] };
			...
			const declaredCoverage =
				...
				(fileCoverage !== undefined
					...
					: { source: name, status: "available" as const, eligible: items?.length ?? 0, extracted: items?.length ?? 0, linked: 0, excluded: 0, missing: 0, reason: "" });
	...
	if (config.sources.commits?.kind === "git" && bundle.commits) {
		...
		if (gitCoverage) {
			if (bundle.tickets) {
				...
				gitCoverage.reason += ` Verified against supplied ticket source: ${gitCoverage.linked} verified links, ` + `${unlinked} unlinked (including ${ambiguous} ambiguous).`;
			} else {
