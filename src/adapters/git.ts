...
function changedFiles(repoPath: string, hash: string): Pick<Commit, 'paths' | 'linesAdded' | 'linesDeleted' |
  'changeSize' | 'testLinesAdded' | 'sourceLinesAdded'> {
  const output = git(repoPath, 'diff-tree', '--root', '-r', '--no-commit-id', '--no-renames',
    '--numstat', '-z', hash);
  const paths: string[] = [];
  let linesAdded = 0;
  let linesDeleted = 0;
  ...
  return numeric ? { paths, linesAdded, linesDeleted, changeSize: linesAdded + linesDeleted,
    testLinesAdded, sourceLinesAdded } : { paths };
}

export function loadLocalGit(repoPath: string, revision: string, since: string, until: string,
  timezone: string, ticketPattern?: string, botAuthors: string[] = []):
  ...
  catch { throw new Error('Git timezone must be valid'); }
  let pattern: RegExp | undefined;
  try { if (ticketPattern !== undefined) pattern = new RegExp(ticketPattern, 'g'); }
  catch { throw new Error('Git ticket pattern must be a valid regular expression'); }
  ...
  } catch { throw new Error('Invalid local Git repository path or revision'); }

  const start = new Date(`${since}T00:00:00Z`);
  const end = new Date(`${until}T00:00:00Z`);
  start.setUTCDate(start.getUTCDate() - 2);
  end.setUTCDate(end.getUTCDate() + 2);
  const output = git(repoPath, 'log', '--no-patch', '-z',
    '--format=%x00%x00%H%x00%P%x00%cI%x00%an%x00%ae%x00%B',
    `--since-as-filter=${start.toISOString()}`, `--until=${end.toISOString()}`, inspectedRevision);
  ...
  while (index < fields.length) {
    ...
    const commit: Commit = {
      hash, provenance: { source: 'git:commits', recordId: hash }, parents, at,
      ...changedFiles(repoPath, hash), bot, ticketIds,
      contributorId: createHmac('sha256', authorKey).update(email || author).digest('hex'),
    };
    commits.push(commit);
    if (bot || parents.length > 1) excluded++;
  }
  const coverage: Coverage = {
    source: 'commits', status: shallow ? 'partial' : 'available', eligible: commits.length - excluded,
    extracted: commits.length, linked: 0, excluded, missing: 0,
    reason: `Inspected revision ${inspectedRevision}.` +
      (shallow ? ' Shallow repository: commit history is incomplete; earlier commits may be missing.' : ''),
  };
