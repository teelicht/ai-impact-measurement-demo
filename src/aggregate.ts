...
export interface OperationalMonth extends OperationalCounts {
	label: string;
	month: string;
	usage: Pick<UsageCostMonth, "inputTokens" | "cachedInputTokens" | "outputTokens" | "byModel" | "attempts" | "workItemLinks"> | null;
...
function firstApproval(ticket: Ticket): string | undefined {
	return ticket.approvals.reduce<string | undefined>(
		(earliest, approval) => (earliest === undefined || Date.parse(approval.at) < Date.parse(earliest) ? approval.at : earliest),
		undefined,
	);
}
...
export function countApprovedParents(tickets: Ticket[], timezone: string): Map<string, number> {
	...
	for (const ticket of tickets) {
		if (ticket.parentId || ticket.emergency || !["Story", "Task"].includes(ticket.type) || ticket.status !== "approved") continue;
...
function combine(rows: OperationalMonth[], label: string, sourceStatus: MeasureStatus): OperationalPeriod {
	...
	const commitLinks = rows.every((row) => row.commitLinks !== null)
		? rows.reduce<CommitLinks>(
				...
				{ linked: 0, unlinked: 0, ambiguous: 0, excluded: 0 },
			)
		: null;
	return {
		label,
		startMonth: rows[0]?.month ?? "",
		endMonth: rows.at(-1)?.month ?? "",
		measureStatus,
		approvedParents: total("approvedParents"),
		createdParents: total("createdParents"),
		openParents: total("openParents"),
		missingApproval: total("missingApproval"),
		missingStartTimes: total("missingStartTimes"),
		incomingBugs: total("incomingBugs"),
		unknownProductionBugs: total("unknownProductionBugs"),
		commits,
		testTouchCommits,
		testTouchShare: commits === null || testTouchCommits === null || commits === 0 ? null : testTouchCommits / commits,
		tokens: total("tokens"),
		toolSpend: total("toolSpend"),
		issueTypes,
		commitLinks,
	};
}

function relativeChange(before: number | null, after: number | null, beforeMonths: number, afterMonths: number): number | null {
	return before === null || after === null || before === 0 ? null : (after * beforeMonths) / (before * afterMonths) - 1;
}
...
export function aggregate(bundle: SourceBundle): OperationalReport {
	...
	const sourceStatus: MeasureStatus = {
		approvedParents: status("tickets"),
		createdParents: status("tickets"),
		openParents: status("tickets"),
		missingApproval: status("tickets"),
		missingStartTimes: status("tickets"),
		incomingBugs: status("tickets"),
		unknownProductionBugs: status("tickets"),
		issueTypes: status("tickets"),
		commits: status("commits"),
		testTouchCommits: status("commits"),
		testTouchShare: status("commits"),
	...
	const missingStartsByMonth = countApprovedParents(
		(bundle.tickets ?? []).filter((ticket) => !ticket.startedAt),
		bundle.timezone,
	);
	...
	const monthly: OperationalMonth[] = months.map((window, index) => {
		...
		const created = bundle.tickets?.filter((ticket) => monthInTimezone(ticket.createdAt, bundle.timezone) === window);
		const parents = created?.filter(eligibleParent);
		const issues =
			created === undefined
				? null
				: created.reduce<Record<string, number>>(
						(counts, ticket) => {
		...
		const relevantCommits = bundle.commits?.filter((commit) => monthInTimezone(commit.at, bundle.timezone) === window);
		...
		const unknownProductionBugs = created?.filter((ticket) => ticket.type === "Bug" && ticket.production === undefined).length ?? null;
		return {
			label: `M${index + 1}`,
			month: window,
			measureStatus: { ...sourceStatus, incomingBugs: unknownProductionBugs && sourceStatus.incomingBugs === "available" ? "partial" : sourceStatus.incomingBugs },
			...
			testTouchShare: commits === null || testTouchCommits === null || commits === 0 ? null : testTouchCommits / commits,
			commitLinks,
			tokens: financeMonth.tokens,
			toolSpend: financeMonth.toolSpend,
			usage:
				bundle.usage === undefined
					? null
					: {
							inputTokens: financeMonth.inputTokens,
							cachedInputTokens: financeMonth.cachedInputTokens,
							outputTokens: financeMonth.outputTokens,
							byModel: financeMonth.byModel,
							attempts: financeMonth.attempts,
							workItemLinks: financeMonth.workItemLinks,
						},
	...
	const periods =
		monthly.length < 2
			? []
			: [combine(monthly.slice(0, splitIndex), `M1-M${splitIndex}`, sourceStatus), combine(monthly.slice(splitIndex), `M${splitIndex + 1}-M${monthly.length}`, sourceStatus)];
	const [before, after] = periods;
	const tickets = bundle.tickets ?? [];
	const commits = bundle.commits ?? [];
	const includedCommits = commits.filter((commit) => !commit.bot && commit.parents.length <= 1 && months.includes(monthInTimezone(commit.at, bundle.timezone)));
	const completeCommits = status("commits") === "available" && includedCommits.length > 0;
	const sizedCommits = includedCommits.filter((commit): commit is Commit & { changeSize: number } => commit.changeSize !== undefined);
	const sizes = completeCommits && sizedCommits.length === includedCommits.length ? sizedCommits.map((commit) => commit.changeSize).sort((left, right) => left - right) : null;
	const addedCommits = includedCommits.filter(
		(
			commit,
		): commit is Commit & {
	...
	const sourceLines = completeCommits && addedCommits.length === includedCommits.length ? addedCommits.reduce((sum, commit) => sum + commit.sourceLinesAdded, 0) : null;
	const diagnostics = {
		churn:
			completeCommits && includedCommits.every((commit) => commit.linesAdded !== undefined && commit.linesDeleted !== undefined)
				? includedCommits.reduce(
						(totals, commit) => ({
				...
				: null,
		contributorCount:
			completeCommits && includedCommits.every((commit) => commit.contributorId !== undefined) ? new Set(includedCommits.map((commit) => commit.contributorId)).size : null,
		meanChangeSize: sizes === null ? null : sizes.reduce((sum, size) => sum + size, 0) / sizes.length,
		medianChangeSize: sizes === null ? null : sizes.length % 2 === 0 ? (sizes[sizes.length / 2 - 1] + sizes[sizes.length / 2]) / 2 : sizes[Math.floor(sizes.length / 2)],
		largeChangeShare:
			sizes === null
				? null
				: sizedCommits.filter(
						(commit) => commit.changeSize > 100 && new Set(commit.paths.map((path) => (path.includes("/") ? path.split("/")[0] : path.split(".").at(-1)))).size >= 2,
					).length / sizedCommits.length,
		testToSourceRatio: sourceLines === null || sourceLines === 0 ? null : addedCommits.reduce((sum, commit) => sum + commit.testLinesAdded, 0) / sourceLines,
	};
	const repositoryDiagnostics =
		diagnostics.churn === null &&
		diagnostics.contributorCount === null &&
		diagnostics.meanChangeSize === null &&
		diagnostics.medianChangeSize === null &&
		diagnostics.largeChangeShare === null &&
		diagnostics.testToSourceRatio === null
			? null
			: diagnostics;
	const diagnosticCoverage: Coverage["status"] =
		repositoryDiagnostics === null ? "unavailable" : Object.values(diagnostics).some((value) => value === null) ? "partial" : "available";
	const definitions = [
		...
		{
			metric: "incomingBugs",
			definition: "Confirmed production Bug tickets grouped by creation month; unknown environments are excluded and reported separately.",
			source: "tickets",
		},
		{ metric: "commits", definition: "Distinct non-merge, non-bot commits by month.", source: "commits" },
		{
			metric: "testTouchShare",
			definition:
				"Eligible commits touching a tests/ or __tests__/ path, a .test/.spec JS/TS file, a _test.go file or a test_*.py file, once per commit, divided by all eligible commits.",
			source: "commits",
		},
		...
		{
			metric: "toolSpendPerApprovedParent",
			definition: "Recorded consumption, allocated subscriptions and other tool charges divided by approved Stories and Tasks when billing and ticket coverage are complete.",
			source: "charges",
		},
		{
			metric: "totalAiCost",
			definition: "Tool charges plus human review, correction, enablement, governance and platform costs; unavailable for the API team.",
			source: "total-ai-cost",
		},
		...
		{ metric: "escapedDefects", definition: "Bugs linked to an approved release with follow-up coverage, not all incoming bugs.", source: "defect-release-linkage" },
		{
			...
			definition:
				"Observed line churn, distinct contributors, median and mean change size, share above 100 changed lines across two areas, and unit-test/source lines added ratio for eligible commits; no contributor identities reported. No move discount or effort inference.",
			source: "commits",
		},
	].map((entry) => ({
		...entry,
		coverage:
			entry.metric === "repositoryDiagnostics"
				...
				: entry.metric === "incomingBugs" && monthly.some((row) => row.measureStatus.incomingBugs === "partial")
					? ("partial" as const)
					: entry.metric === "tokens" || entry.metric === "toolSpend"
						? sourceStatus[entry.metric]
						: status(entry.source),
	}));
	...
	const approvedTickets = eligible.flatMap((ticket) => {
		...
		const first = firstApproval(ticket);
		if (!first || dateInTimezone(first, bundle.timezone) < bundle.startDate || dateInTimezone(first, bundle.timezone) > bundle.endDate) return [];
		return [{ ticket, first }];
	});
	const missingStarts = approvedTickets.filter(({ ticket }) => !ticket.startedAt).length;
	const leadTime =
		approvedTickets.length > 0 && missingStarts === 0 && status("tickets") === "available" && status("start-times") === "available"
	...
	if (unknownProductionBugs > 0) {
		evidenceGaps.push({ metric: "incomingBugs", reason: `${unknownProductionBugs} Bug tickets have unknown production environment.` });
	}
	if (leadTime === null) {
		evidenceGaps.push({
			metric: "leadTime",
	...
	evidenceGaps.push({
		metric: "humanEffort",
		reason: coverage.find((entry) => entry.source === "historical-effort")?.reason || "Observed non-overlapping person-minutes by activity are not available.",
	});
	evidenceGaps.push({
		metric: "escapedDefects",
		reason: coverage.find((entry) => entry.source === "defect-release-linkage")?.reason || "Bug-to-release links and follow-up coverage are not available.",
	});
	...
	if (missingDiagnostics.length > 0) {
		evidenceGaps.push({ metric: "repositoryDiagnostics", reason: `Complete eligible-commit observations unavailable for ${missingDiagnostics.join(", ")} diagnostics.` });
	}
	evidenceGaps.push({ metric: "releaseDiagnostics", reason: "Approval links provide release IDs but no release history or completion events." });
	...
	return {
		...
		completenessAttestation: bundle.completenessAttestation,
		...
		periods,
		comparisons: {
			approvedParentsRelativeChange:
				before && after && before.measureStatus.approvedParents === "available" && after.measureStatus.approvedParents === "available"
					? relativeChange(before.approvedParents, after.approvedParents, splitIndex, monthly.length - splitIndex)
					: null,
			commitsRelativeChange:
				before && after && before.measureStatus.commits === "available" && after.measureStatus.commits === "available"
					? relativeChange(before.commits, after.commits, splitIndex, monthly.length - splitIndex)
					: null,
			incomingBugsRelativeChange:
				before && after && before.measureStatus.incomingBugs === "available" && after.measureStatus.incomingBugs === "available"
					? relativeChange(before.incomingBugs, after.incomingBugs, splitIndex, monthly.length - splitIndex)
					: null,
			testTouchSharePercentagePoints:
				!before ||
				!after ||
				before.measureStatus.testTouchShare !== "available" ||
				after.measureStatus.testTouchShare !== "available" ||
				before.testTouchShare === null ||
				after.testTouchShare === null
					? null
					: (after.testTouchShare - before.testTouchShare) * 100,
		},
		coverage,
		definitions,
		exclusions: {
			...
			nonEligibleTypes: tickets.filter((ticket) => !ticket.parentId && !ticket.emergency && !eligibleParent(ticket)).length,
		...
		humanEffort: null,
		escapedDefects: null,
		totalAiCost: accounting.totalAiCost,
		roi: accounting.roi,
		releaseDiagnostics: null,
		repositoryDiagnostics,
	};
