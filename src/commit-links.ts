...
export function classifyCommitLink(commit: Commit, knownTickets: ReadonlySet<string>): "linked" | "unlinked" | "ambiguous" {
	const ids = new Set(commit.ticketIds);
	return ids.size > 1 ? "ambiguous" : ids.size === 1 && knownTickets.has([...ids][0]) ? "linked" : "unlinked";
}
