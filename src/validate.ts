...
function date(value: unknown, label: string): string {
	...
	if (!/^\d{4}-\d{2}-\d{2}$/.test(valueText) || Number.isNaN(Date.parse(`${valueText}T00:00:00Z`)) || new Date(`${valueText}T00:00:00Z`).toISOString().slice(0, 10) !== valueText) {
...
export function validateBundle(bundle: SourceBundle): SourceBundle {
	...
	if (bundle.context !== undefined) {
		...
		const contextFields = ["owner", "description", "includedWork", "excludedWork", "aiUse", "concurrentChanges", "question", "controls", "resourceLimits"] as const;
		...
		if (bundle.context.repositories !== undefined) {
			...
			bundle.context.repositories.forEach((label, index) => {
				...
				if (/[/\\@]/.test(name) || /[a-z][a-z0-9+.-]*:/i.test(name) || Array.from(name).some((character) => character.charCodeAt(0) < 32 || character.charCodeAt(0) === 127)) {
	...
	for (const entry of bundle.coverage) {
		...
		if (typeof entry.reason !== "string" || (entry.status !== "available" && entry.reason.trim() === "")) {
	...
	for (const event of bundle.usage ?? []) {
		...
		if (isMonthlyUsage(event)) {
			...
			if (["model", "at", "inputTokens", "outputTokens", "cachedInputTokens", "attempt", "outcome", "ticketId"].some((field) => field in event))
		...
		if (!Number.isSafeInteger(event.cachedInputTokens) || event.cachedInputTokens < 0 || event.cachedInputTokens > event.inputTokens) {
	...
	if (hasRollups) {
		const months = (Number(endDate.slice(0, 4)) - Number(startDate.slice(0, 4))) * 12 + Number(endDate.slice(5, 7)) - Number(startDate.slice(5, 7)) + 1;
		const missingMonths = months - rollupMonths.size;
		const usageCoverage = bundle.coverage.find((entry) => entry.source === "usage");
		if (
			!usageCoverage ||
			usageCoverage.missing !== missingMonths ||
			usageCoverage.eligible !== months ||
			usageCoverage.linked !== 0 ||
			usageCoverage.excluded !== 0 ||
