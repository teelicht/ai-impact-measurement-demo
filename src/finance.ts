import { type Charge, type Coverage, isMonthlyUsage, type SourceBundle } from "./model.js";
...
export function reconcileCharges(bundle: SourceBundle): Charge[] {
	...
	for (const charge of bundle.charges ?? []) {
		...
		if (
			!charge.billId?.trim() ||
			!Number.isFinite(charge.billTotal) ||
			charge.billTotal < 0 ||
			!charge.allocatedTo?.trim() ||
			!charge.allocationKey?.trim() ||
...
export function summarizeUsageAndCost(bundle: SourceBundle): UsageCostSummary {
	...
	for (const event of bundle.usage ?? []) {
		...
		if (isMonthlyUsage(event)) {
			if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(event.month) || !Number.isSafeInteger(event.tokens) || event.tokens < 0) {
		...
		if (
			...
			!Number.isSafeInteger(event.cachedInputTokens) ||
	...
	if (hasRollups) {
		...
		if (
			coverage?.missing !== missingMonths ||
			coverage.eligible !== expectedMonths ||
			coverage.linked !== 0 ||
			coverage.excluded !== 0 ||
	...
	const evidenceGaps = (["usage", "charges", "tickets"] as const).flatMap((source) => {
		...
		const reason =
			bundle[source]?.length === 0
				? "No records in the source export; verify completeness."
	...
	const monthly = reportingMonths(bundle.startDate, bundle.endDate).map((month) => ({
		month,
		measureStatus: {
			tokens: sourceStatus("usage"),
			toolSpend: sourceStatus("charges"),
			approvedParents: sourceStatus("tickets"),
			toolSpendPerApprovedParent:
				sourceStatus("tickets") === "unavailable" || sourceStatus("charges") === "unavailable"
	...
	for (const ticket of bundle.tickets ?? []) {
		if (ticket.parentId || ticket.emergency || !["Story", "Task"].includes(ticket.type) || ticket.status !== "approved" || ticket.approvals.length === 0) continue;
		const first = ticket.approvals.reduce((earliest, approval) => (Date.parse(approval.at) < Date.parse(earliest) ? approval.at : earliest), ticket.approvals[0].at);
	...
	for (const row of monthly) {
		if (row.measureStatus.toolSpendPerApprovedParent === "available" && row.approvedParents !== null && row.approvedParents > 0 && row.toolSpend !== null) {
	...
	return {
		...
		netBenefit: null,
		roi: null,
		financialGaps: ["Recorded tool spend excludes unpriced human review, correction, enablement, governance and platform costs; total AI cost and API-team ROI are unassessed."],
	};
}
...
export function evaluateFinancialCase(caseData: FinancialCase): FinancialResult {
	...
	for (const claim of claims) {
		...
		if (
			claim.verificationStatus === "excluded" &&
			claim.category === "overlapping-benefit" &&
			claim.overlapsWith &&
			claims.some((other) => other.id === claim.overlapsWith && ["avoided-cost", "realized-value"].includes(other.category))
		) {
	...
	if (gaps.length > 0) {
		return { currency: caseData.currency, period: caseData.period, realizedValue: null, avoidedCost: null, totalCost: null, netBenefit: null, roi: null, excludedClaims, gaps };
	}
	const netBenefit = realizedValue + avoidedCost - totalCost;
	return { currency: caseData.currency, period: caseData.period, realizedValue, avoidedCost, totalCost, netBenefit, roi: netBenefit / totalCost, excludedClaims, gaps };
}
