...
import { evaluateFinancialCase, summarizeUsageAndCost, type FinancialCase } from '../src/finance.js';
import type { UsageEvent } from '../src/model.js';
import { isMonthlyUsage } from '../src/model.js';

const detailedEvent: UsageEvent = {
  id: 'use-1', provenance: { source: 'test', recordId: 'use-1' },
  at: '2025-09-18T10:00:00Z', model: 'example-model',
  inputTokens: 100, cachedInputTokens: 40, outputTokens: 20,
  attempt: 1, outcome: 'success', ticketId: 'API-1000',
};
...
test('failed attempts and retries each count once in model and status breakdowns', () => {
  const source = loadSynthetic();
  const summary = summarizeUsageAndCost({ ...source, usage: [
    { ...detailedEvent, outcome: 'failed', attempt: 1 },
    { ...detailedEvent, id: 'retry', provenance: { source: 'test', recordId: 'retry' },
      model: 'second-model', inputTokens: 50, cachedInputTokens: 10, outputTokens: 30,
  ...
  assert.deepEqual(month.byModel['example-model'], {
    inputTokens: 100, cachedInputTokens: 40, outputTokens: 20, tokens: 120,
    attempts: 1, failed: 1, retries: 0,
  });
...
test('monthly rollups retain total tokens but have no inferred breakdown', () => {
  ...
  assert.throws(() => summarizeUsageAndCost({ ...source,
    usage: [...records, detailedEvent] }), /mixed usage modes/i);
  assert.throws(() => summarizeUsageAndCost({ ...source,
    usage: [...records, { ...first, id: 'another-id' }] }), /duplicate month/i);
});
...
test('a missing month in partial monthly usage is unavailable, not zero', () => {
  ...
  const records = source.usage;
  assert.ok(records);
  const summary = summarizeUsageAndCost({ ...source, usage: records.slice(1),
    coverage: source.coverage.map(entry => entry.source === 'usage'
      ? { ...entry, status: 'partial', extracted: 11, missing: 1,
        reason: 'September token total not supplied.' } : entry),
  });
...
test('direct monthly accounting rejects out-of-window rollups and empty exports remain unknown', () => {
  ...
  assert.throws(() => summarizeUsageAndCost({ ...source,
    usage: [{ ...first, month: '2024-09' }, ...records.slice(1)] }), /outside|window/i);
...
test('unavailable or partial sources cannot become a zero or complete cost ratio', () => {
  const source = loadSynthetic();
  const summary = summarizeUsageAndCost({ ...source, usage: undefined, charges: undefined,
    coverage: source.coverage.map(entry => entry.source === 'usage' || entry.source === 'charges'
      ? { ...entry, status: 'unavailable' as const, extracted: 0, linked: 0, reason: 'Not supplied.' }
      : entry),
  });
  assert.equal(summary.monthly[0].tokens, null);
  ...
  const partial = summarizeUsageAndCost({ ...source, coverage: source.coverage.map(entry =>
    entry.source === 'tickets' ? { ...entry, status: 'partial' as const, reason: 'Missing tickets.' } : entry) });
...
test('rejects duplicate usage IDs, monthly allocations, and conflicting charge currencies', () => {
  ...
  assert.throws(() => summarizeUsageAndCost({ ...source, charges: [charge, { ...charge,
    id: 'another', provenance: { source: 'test', recordId: 'another' },
...
test('different allocation keys cannot charge more than their shared bill total', () => {
  ...
  const second = { ...first, id: 'second-allocation',
    provenance: { source: 'test', recordId: 'second-allocation' },
    allocationKey: 'other-team', amount: 60 };
  assert.throws(() => summarizeUsageAndCost({ ...source, charges: [first, second] }),
    /shared-bill.*(total|allocat)/i);
});
...
test('a valid shared bill counts only this team allocations once', () => {
  ...
  const charges = [bill, { ...bill, id: 'second-team-charge',
    provenance: { source: 'test', recordId: 'second-team-charge' },
    allocationKey: 'team-extra', amount: 15 }, { ...bill, id: 'other-team-charge',
    provenance: { source: 'test', recordId: 'other-team-charge' },
    allocatedTo: 'Other team', allocationKey: 'other-team', amount: 60 }];
...
test('direct summaries reject partial March windows with a whole-month charge', () => {
  ...
  for (const [startDate, endDate] of [
    ...
    assert.throws(() => summarizeUsageAndCost({ ...source, startDate, endDate, charges: [charge] }),
      /full month/i);
  }
});
...
test('direct summaries expose gaps and incomplete status for partial and empty sources', () => {
  const source = loadSynthetic();
  const partial = summarizeUsageAndCost({ ...source, usage: [], charges: [],
    coverage: source.coverage.map(entry => entry.source === 'usage' || entry.source === 'charges'
...
test('model and billing labels named like object prototype properties remain ordinary data', () => {
  ...
  const charge = source.charges?.[0];
  assert.ok(charge);
  const month = summarizeUsageAndCost({ ...source,
    usage: [{ ...detailedEvent, model: '__proto__' }],
    charges: [{ ...charge, category: '__proto__', amount: 20 }],
  }).monthly[0];
...
test('duplicate benefit IDs, verified overlapping benefits, or unapproved cancellation block ROI', () => {
  const duplicate = { ...migration, claims: [...migration.claims,
    { ...migration.claims[0], category: 'realized-value' as const, amount: 2400 }] };
  ...
  const overlap = { ...migration, claims: migration.claims.map(claim =>
    claim.id === 'MIG-DUP-01' ? { ...claim, verificationStatus: 'verified' as const } : claim) };
  assert.equal(evaluateFinancialCase(overlap).netBenefit, null);
  assert.match(evaluateFinancialCase(overlap).gaps.join(' '), /overlap/i);
  const cancellation = { ...migration, claims: migration.claims.map(claim =>
    claim.id === 'MIG-01' ? { ...claim, cancellationApproved: false } : claim) };
  ...
  const scope = { ...migration, claims: migration.claims.map(claim =>
    claim.id === 'MIG-01' ? { ...claim, scopeAccepted: false } : claim) };
...
test('distinct IDs cannot double-claim the same verified source allocation', () => {
  const duplicateCost = { ...migration, claims: [...migration.claims,
    { ...migration.claims[1], id: 'MIG-COST-COPY' }] };
...
test('conflicting currency or period and incomplete or unverified costs block both financial results', () => {
  for (const caseData of [
    { ...migration, claims: migration.claims.map(claim => claim.id === 'MIG-COST-01'
      ? { ...claim, currency: 'EUR' } : claim) },
    { ...migration, claims: migration.claims.map(claim => claim.id === 'MIG-COST-01'
      ? { ...claim, period: '6 weeks' } : claim) },
    { ...migration, totalIncrementalCostComplete: false },
    { ...migration, claims: migration.claims.map(claim => claim.id === 'MIG-COST-01'
      ? { ...claim, verificationStatus: 'unverified' as const } : claim) },
  ]) {
