...
const withoutUsage = (source: SourceBundle): SourceBundle => ({
	...
	coverage: source.coverage.map((entry) =>
		entry.source === "usage"
			? { source: "usage", status: "unavailable", eligible: 0, extracted: 0, linked: 0, excluded: 0, missing: 0, reason: "Usage not in this comparison fixture." }
			: entry,
	),
});
...
test("a fourteen-month configured window compares every reported month in adjacent halves", () => {
	...
	assert.deepEqual(
		report.periods.map((row) => [row.label, row.startMonth, row.endMonth]),
		[
			["M1-M7", "2025-09", "2026-03"],
			["M8-M14", "2026-04", "2026-10"],
		],
	);
...
test("a period rejects token totals that exceed safe integer precision", () => {
	...
	assert.ok(rollups);
	const usage = rollups.map((record, index) => ({ ...record, tokens: index === 0 ? Number.MAX_SAFE_INTEGER : index === 1 ? 2 : 0 }));
...
test("prototype-like issue types survive monthly and period grouping without changing totals", () => {
	...
	const expected = ['{"__proto__":1,"constructor":1,"All created issues":1}', '{"__proto__":1,"toString":1}'];
	...
	assert.deepEqual(JSON.parse(JSON.stringify(report.periods[0].issueTypes)), JSON.parse('{"__proto__":2,"constructor":1,"toString":1,"All created issues":1}'));
	...
	assert.equal(
		Object.values(report.periods[0].issueTypes ?? {}).reduce((sum, value) => sum + value, 0),
		5,
	);
...
test("incoming production bugs exclude nonproduction and qualify unknown imports", () => {
	const source = loadSynthetic();
	const bugs = [
		ticket("prod", { type: "Bug", status: "unknown", approvals: [], production: true, createdAt: "2026-03-11T00:00:00Z" }),
		ticket("nonprod", { type: "Bug", status: "unknown", approvals: [], production: false, createdAt: "2026-03-11T00:00:00Z" }),
		ticket("unspecified", { type: "Bug", status: "unknown", approvals: [], createdAt: "2026-03-11T00:00:00Z" }),
	];
	...
	assert.ok(report.evidenceGaps.some((gap) => gap.metric === "incomingBugs" && gap.reason.includes("production")));
...
test("complete observed commit diagnostics aggregate without exposing contributor identities", () => {
	...
	const commits: Commit[] = [
		...
		{ ...first, hash: "second", linesAdded: 8, linesDeleted: 6, contributorId: "b@example.org", changeSize: 30 },
		{ ...first, hash: "merge", parents: ["one", "two"], linesAdded: 1000, linesDeleted: 1000, contributorId: "excluded@example.org", changeSize: 2000 },
	];
	const report = aggregate({ ...source, commits });
	assert.deepEqual(report.repositoryDiagnostics, {
		churn: { linesAdded: 20, linesDeleted: 10 },
		contributorCount: 2,
		meanChangeSize: 20,
		medianChangeSize: 20,
		largeChangeShare: 0,
...
test("repository diagnostics definition reflects optional observation completeness", () => {
	...
	const first = source.commits?.[0];
	assert.ok(first);
	const definitionCoverage = (commits: Commit[]) => aggregate({ ...source, commits }).definitions.find((entry) => entry.metric === "repositoryDiagnostics")?.coverage;
	assert.equal(aggregate(source).definitions.find((entry) => entry.metric === "repositoryDiagnostics")?.coverage, "unavailable");
	const complete: Commit[] = [
		{ ...first, linesAdded: 12, linesDeleted: 4, contributorId: "person-1", changeSize: 10, testLinesAdded: 0, sourceLinesAdded: 12 },
		{ ...first, hash: "second", linesAdded: 8, linesDeleted: 6, contributorId: "person-2", changeSize: 30, testLinesAdded: 0, sourceLinesAdded: 8 },
	];
	assert.equal(definitionCoverage([complete[0], { ...first, hash: "second", contributorId: "person-2" }]), "partial");
...
test("lead time requires start evidence for every approved parent", () => {
	...
	const complete = aggregate({
		...
		coverage: source.coverage.map((entry) =>
			entry.source === "start-times" ? { ...entry, status: "available" as const, eligible: 2, extracted: 2, linked: 2, missing: 0, reason: "" } : entry,
		),
	});
	assert.equal(complete.leadTime, 6);
	assert.equal(complete.monthly[6].missingStartTimes, 0);
	assert.equal(
		complete.evidenceGaps.some((gap) => gap.metric === "leadTime"),
		false,
	);
});
...
test("lead time uses first approval in the reporting window, consistent with approval ordering", () => {
	...
	const report = aggregate({
		...source,
		tickets: [inWindow, outside],
		coverage: source.coverage.map((entry) =>
			entry.source === "start-times" ? { ...entry, status: "available" as const, eligible: 1, extracted: 1, linked: 1, missing: 0, reason: "" } : entry,
		),
	});
	assert.equal(report.leadTime, 2);
	assert.equal(
		report.evidenceGaps.some((gap) => gap.metric === "leadTime"),
		false,
	);
	assert.throws(() => validateBundle({ ...source, tickets: [{ ...inWindow, approvals: [...inWindow.approvals].reverse() }] }), /approvals must follow work events in order/);
});
...
test("lead time is unknown when ticket coverage is partial despite observed starts", () => {
	...
	const report = aggregate({
		...
		coverage: source.coverage.map((entry) =>
			entry.source === "tickets"
				? { ...entry, status: "partial" as const, reason: "Some tickets were not imported." }
				: entry.source === "start-times"
					? { ...entry, status: "available" as const, eligible: 1, extracted: 1, linked: 1, missing: 0, reason: "" }
					: entry,
		),
	});
	assert.equal(report.leadTime, null);
	assert.ok(report.evidenceGaps.some((gap) => gap.metric === "leadTime" && gap.reason.includes("ticket")));
});
