...
test("default CLI writes one synthetic report as HTML and evidence JSON", () => {
	...
	try {
		...
		assert.deepEqual(readdirSync(output).sort(), ["api-team.html", "api-team-evidence.json"].sort());
		...
		assert.equal(
			rows.slice(0, 6).reduce((sum: number, row: { approvedParents: number }) => sum + row.approvedParents, 0),
			126,
		);
		assert.equal(
			rows.slice(6).reduce((sum: number, row: { approvedParents: number }) => sum + row.approvedParents, 0),
			168,
		);
...
test("a missing configured input fails without creating synthetic output", () => {
	...
	try {
		const result = spawnSync(process.execPath, [cli, "--config", join(output, "missing.json"), "--out", output], { encoding: "utf8" });
...
test("configured ticket exports keep their actual window and unavailable sources", () => {
	...
	try {
		const result = spawnSync(process.execPath, [cli, "--config", config, "--out", output], { encoding: "utf8" });
		assert.equal(result.status, 0, result.stderr);
		const html = readFileSync(join(output, "example-api-team.html"), "utf8");
		assert.deepEqual(readdirSync(output).sort(), ["example-api-team.html", "example-api-team-evidence.json"].sort());
...
test("configured report metadata appears in HTML and evidence without synthetic fallback", () => {
	...
	try {
		...
		configuration.context = { owner: "Service lead", updatedAt: "2026-04-02", description: "Fictional checkout service", repositories: ["Synthetic checkout history"] };
		const configPath = join(output, "config.json");
		writeFileSync(configPath, JSON.stringify(configuration));
		const result = spawnSync(process.execPath, [cli, "--config", configPath, "--out", output], { encoding: "utf8" });
		assert.equal(result.status, 0, result.stderr);
		const html = readFileSync(join(output, "example-api-team.html"), "utf8");
		const evidence = JSON.parse(readFileSync(join(output, "example-api-team-evidence.json"), "utf8"));
...
test("one-month partial incoming bugs retain the numeric count and status in evidence", () => {
	...
	try {
		...
		writeFileSync(
			join(output, "tickets.json"),
			JSON.stringify({
				version: 1,
				tickets: [
					{ id: "BUG-1", type: "Bug", createdAt: "2026-03-05T09:00:00Z", production: true, statusEvents: [] },
					{ id: "BUG-2", type: "Bug", createdAt: "2026-03-06T09:00:00Z", statusEvents: [] },
				],
			}),
		);
		const result = spawnSync(process.execPath, [cli, "--config", join(output, "config.json"), "--out", output], { encoding: "utf8" });
		assert.equal(result.status, 0, result.stderr);
		const evidence = JSON.parse(readFileSync(join(output, "api-team-evidence.json"), "utf8"));
		assert.equal(evidence.monthly.length, 1);
		assert.equal(evidence.monthly[0].month, "2026-03");
...
test("repeating --out rejects the default-valued first occurrence before writing files", () => {
	...
	try {
		const result = spawnSync(process.execPath, [cli, "--out", "output", "--out", "another"], { encoding: "utf8", cwd: output });
		assert.notEqual(result.status, 0);
...
test("configured exports use safe scope slugs and a predictable fallback for empty slugs", () => {
	...
	try {
		for (const [scope, base] of [
			...
			const result = spawnSync(process.execPath, [cli, "--config", configPath, "--out", output], { encoding: "utf8" });
			assert.equal(result.status, 0, result.stderr);
			assert.deepEqual(readdirSync(output).sort(), [`${base}.html`, `${base}-evidence.json`].sort());
