...
test("validates optional report context without confusing update and extraction dates", () => {
	const context = {
		...
		aiUse: "Agent-supported endpoint preparation",
	...
	for (const invalid of [
		...
		assert.throws(() => validateBundle({ ...valid, context: invalid } as SourceBundle), /context|repositories|updatedAt/i, JSON.stringify(invalid));
	}
});
...
const rollupBundle = (): SourceBundle => ({
	...valid,
	usage: monthlyUsage,
	coverage: valid.coverage.map((entry) => (entry.source === "usage" ? { ...entry, eligible: 12, extracted: 12, linked: 0 } : entry)),
});
...
test("accepts complete monthly token totals without invented event categories", () => {
	...
	assert.equal(
		bundle.usage?.reduce((sum, record) => sum + ("tokens" in record ? record.tokens : 0), 0),
		78_000_000,
	);
});
	...
	assert.throws(
		() =>
			validateBundle({
				...
				coverage: valid.coverage.map((entry) => (entry.source === "usage" ? { ...entry, eligible: 13, extracted: 13, linked: 0 } : entry)),
			}),
		/mixed|one mode/i,
	);
	assert.throws(
		() =>
			validateBundle({
				...rollupBundle(),
				usage: [
					...monthlyUsage.slice(0, 11),
					{ ...monthlyUsage[11], id: "different-id", provenance: { source: "monthly-usage-export", recordId: "different-id" }, month: "2026-11" },
				],
			}),
		/duplicate.*month/i,
	);
...
test("rejects malformed rollups and missing months falsely declared available", () => {
	...
	assert.throws(
		() =>
			validateBundle({
				...rollupBundle(),
				usage: monthlyUsage.slice(1),
				coverage: valid.coverage.map((entry) => (entry.source === "usage" ? { ...entry, eligible: 11, extracted: 11, linked: 0 } : entry)),
			}),
		/missing|month|complete/i,
	);
});

test("empty or overdeclared monthly usage cannot claim complete coverage", () => {
	const empty = { ...valid, usage: [], coverage: valid.coverage.map((entry) => (entry.source === "usage" ? { ...entry, eligible: 0, extracted: 0, linked: 0 } : entry)) };
	assert.throws(() => validateBundle(empty), /usage.*(empty|records|available)/i);
	const exaggerated = {
		...rollupBundle(),
		usage: monthlyUsage.slice(1),
		coverage: valid.coverage.map((entry) =>
			entry.source === "usage" ? { ...entry, status: "partial" as const, eligible: 13, extracted: 11, linked: 0, missing: 2, reason: "September unavailable." } : entry,
		),
	};
...
test("rejects duplicate provenance within each record category even when local IDs differ", () => {
	for (const source of ["tickets", "commits", "usage", "charges"] as const) {
		...
		assert.throws(
			...
			new RegExp(`${source} .*: provenance.*duplicate`),
...
test("rejects cached input exceeding input tokens with a record ID", () => {
	assert.throws(
		() =>
			validateBundle({
				...valid,
				usage: [{ ...usage, cachedInputTokens: 101 }],
			}),
	...
	for (const outcome of ["", "failure", "pending", 0, null]) {
		assert.throws(
			() =>
				validateBundle({
					...valid,
					usage: [{ ...usage, outcome } as UsageEvent],
				}),
			/usage usage-1: outcome/,
...
test("rejects invalid ISO dates on records and metadata", () => {
	assert.throws(
		() =>
			validateBundle({
				...valid,
				tickets: [{ ...valid.tickets![0], createdAt: "2026-02-30T08:00:00Z" }],
			}),
	...
	for (const status of ["", "pending", 0, null]) {
		assert.throws(
			() =>
				validateBundle({
					...valid,
					tickets: [{ ...approvedTicket, status } as Ticket],
				}),
	...
	for (const status of ["open", "unknown"] as const) {
		assert.throws(
			() =>
				validateBundle({
					...valid,
					tickets: [{ ...approvedTicket, status }],
				}),
			/tickets API-1: status/,
			`${status} with an approval`,
		);
	}
	assert.throws(
		() =>
			validateBundle({
				...valid,
				tickets: [{ ...approvedTicket, status: "approved", approvals: [] }],
			}),
		/tickets API-1: status/,
	);
	for (const status of ["open", "unknown"] as const) {
...
test("validates explicit production flags while permitting unknown imported bug environments", () => {
	...
	for (const production of ["yes", 1, null]) {
		assert.throws(() => validateBundle({ ...valid, tickets: [{ ...base, type: "Bug", production } as unknown as Ticket] }), /tickets API-1: production/);
	}
});
...
test("validates optional observed commit diagnostics", () => {
	...
	assert.ok(base);
	const observed: SourceBundle = { ...valid, commits: [{ ...base, linesAdded: 12, linesDeleted: 4, contributorId: "person-1", changeSize: 16 }] };
	...
	for (const field of ["linesAdded", "linesDeleted", "changeSize"] as const) {
		for (const value of [-1, 1.5, "12"]) {
			assert.throws(() => validateBundle({ ...valid, commits: [{ ...base, [field]: value }] }), new RegExp(`commits abc123: ${field}`));
		}
	}
	assert.throws(() => validateBundle({ ...valid, commits: [{ ...base, contributorId: " " }] }), /commits abc123: contributorId/);
});
...
test("rejects charge currency different from bundle currency", () => {
	assert.throws(
		() =>
			validateBundle({
				...valid,
				charges: [{ ...valid.charges![0], currency: "USD" }],
			}),
...
test("accepts absent optional arrays only when coverage is unavailable", () => {
	const unavailable = {
		...
		coverage: valid.coverage.map((entry) =>
			entry.source === "usage" || entry.source === "charges"
				? { ...entry, status: "unavailable" as const, eligible: 0, extracted: 0, linked: 0, excluded: 0, missing: 0, reason: "No export provided" }
				: entry,
		),
	};
	...
	assert.throws(
		() =>
			validateBundle({
				...unavailable,
				coverage: unavailable.coverage.map((entry) => (entry.source === "usage" ? { ...entry, status: "partial" as const } : entry)),
			}),
		/usage:.*coverage/,
	);
});
...
test("rejects repeated monthly allocation keys but permits recurring keys in other months", () => {
	...
	const bundle = { ...valid, coverage: valid.coverage.map((entry) => (entry.source === "charges" ? { ...entry, eligible: 2, extracted: 2 } : entry)) };
	assert.throws(
		() =>
			validateBundle({
				...bundle,
	...
	assert.strictEqual(
		validateBundle({
			...bundle,
			charges: [first, { ...second, billId: "bill-2", month: "2026-02" }],
		}).charges?.length,
		2,
	);
});
...
test("validates shared bill totals across distinct allocation keys and consistent bill identity", () => {
	...
	const second = { ...first, id: "charge-2", provenance: { source: "billing-export", recordId: "charge-2" }, allocationKey: "other-team", allocatedTo: "Other team", amount: 60 };
	const bundle = { ...valid, coverage: valid.coverage.map((entry) => (entry.source === "charges" ? { ...entry, eligible: 2, extracted: 2 } : entry)) };
	assert.strictEqual(validateBundle({ ...bundle, charges: [first, second] }).charges?.length, 2);
	assert.throws(() => validateBundle({ ...bundle, charges: [first, { ...second, amount: 61 }] }), /bill-1.*(total|allocat)/i);
	assert.throws(() => validateBundle({ ...bundle, charges: [first, { ...second, billTotal: 101 }] }), /bill-1.*metadata/i);
...
test("one bill can allocate across categories and full reporting months without duplicating its total", () => {
	...
	const allocations = [
		{ ...first, billTotal: 100, amount: 60 },
		{
			...
			allocationKey: "february-consumption",
			allocatedTo: "Other team",
			billTotal: 100,
			amount: 40,
		},
	];
	...
	assert.throws(() => validateBundle({ ...bundle, charges: [allocations[0], { ...allocations[1], amount: 41 }] }), /bill-1.*(total|allocat)/i);
});
...
test("rejects missing parents and approval events before work began", () => {
	assert.throws(
		() =>
			validateBundle({
				...valid,
				tickets: [{ ...valid.tickets![0], parentId: "API-missing" }],
			}),
	...
	assert.throws(
		() =>
			validateBundle({
				...valid,
				tickets: [{ ...valid.tickets![0], approvals: [{ at: "2026-01-01T10:00:00Z", releaseId: "R1" }] }],
			}),
...
test("rejects invalid coverage counts and missing source coverage", () => {
	assert.throws(
		() =>
			validateBundle({
				...valid,
				coverage: valid.coverage.map((entry) => (entry.source === "usage" ? { ...entry, missing: -1 } : entry)),
			}),
	...
	assert.throws(
		() =>
			validateBundle({
				...valid,
				coverage: valid.coverage.filter((entry) => entry.source !== "usage"),
			}),
		/usage:.*coverage/,
	);
});
...
test("shared coverage extracted counts match returned records", () => {
	for (const sourceKind of ["synthetic", "configured"] as const) {
		const bundle = structuredClone(valid);
		bundle.sourceKind = sourceKind;
		bundle.coverage = bundle.coverage.map((entry) => (entry.source === "usage" ? { ...entry, eligible: 2, extracted: 2 } : entry));
...
test("rejects contradictory coverage totals without requiring every extracted record to be linked", () => {
	...
	for (const counts of cases) {
		const bundle = structuredClone(valid);
		bundle.coverage = bundle.coverage.map((entry) => (entry.source === "usage" ? { ...entry, ...counts, reason: counts.status === "unavailable" ? "No export" : "" } : entry));
		if (counts.status === "unavailable") bundle.usage = undefined;
		assert.throws(() => validateBundle(bundle), /coverage usage: (extracted|linked|excluded)/, JSON.stringify(counts));
	}
	const bundle = structuredClone(valid);
	bundle.coverage = bundle.coverage.map((entry) =>
		entry.source === "usage" ? { ...entry, status: "partial", eligible: 2, extracted: 1, linked: 0, excluded: 0, missing: 1, reason: "One eligible event not exported" } : entry,
	);
	assert.strictEqual(validateBundle(bundle), bundle);
});
