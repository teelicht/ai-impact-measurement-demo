...
test('versioned targets reproduce every appendix row and both period totals', () => {
  ...
  const sum = (rows: MonthlyRow[], field: keyof Omit<MonthlyRow, 'month'>) =>
  ...
  for (const [rows, totals] of [
    ...
    assert.deepEqual(
      (['approved', 'commits', 'testTouches', 'bugs', 'tokensMillions', 'toolSpendEur'] as const)
        .map(field => sum(rows, field)), totals,
    );
  }
});
...
test('expanded records reconcile to each monthly target and are reproducible', () => {
  ...
  assert.equal(bundle.tickets?.some(ticket => ticket.startedAt !== undefined), false);
  for (const [index, row] of monthly.rows.entries()) {
    const month = new Date(Date.UTC(2025, 8 + index, 1)).toISOString().slice(0, 7);
    const approved: Ticket[] = bundle.tickets!.filter(ticket => !ticket.parentId &&
      ['Story', 'Task'].includes(ticket.type) && ticket.status === 'approved' &&
      ticket.approvals[0]?.at.startsWith(month));
    ...
    assert.equal(commits.filter(commit => commit.paths.some(path => path.includes('.test.'))).length,
      row.testTouches, `${row.month} test touches`);
    assert.equal(bundle.tickets!.filter(ticket => ticket.type === 'Bug' &&
      ticket.createdAt.startsWith(month)).length, row.bugs, `${row.month} bugs`);
    assert.deepEqual(bundle.usage?.filter(isMonthlyUsage).filter(record => record.month === month)
      .map(record => record.tokens), [row.tokensMillions * 1_000_000], `${row.month} monthly tokens`);
    assert.equal(bundle.charges!.filter(charge => charge.month === month).reduce((total, charge) =>
      total + charge.amount, 0), row.toolSpendEur, `${row.month} spend`);
  }
...
test('each month has only one token total and no invented event categories', () => {
  ...
  assert.ok(events.every(event => !('model' in event) && !('cachedInputTokens' in event) &&
    !('outcome' in event) && !('ticketId' in event)));
});
...
test('M7 first commit has a pinned SHA256 hash and follows the last M6 commit', () => {
  ...
  const firstM7 = commits.find(commit => commit.at.startsWith('2026-03') &&
    commit.ticketIds.includes('API-103'));
...
test('March inclusion decisions, attempt accounting and source gaps remain inspectable', () => {
  ...
  assert.equal(tickets.filter(item => item.type === 'Bug').every(item =>
    item.status === 'unknown'), true);
  assert.equal(bundle.commits!.some(commit => commit.ticketIds.includes('API-103') &&
    commit.paths.some(path => path.includes('.test.'))), true);
  assert.deepEqual(bundle.usage!.filter(isMonthlyUsage).filter(event => event.month === '2026-03')
    .map(event => event.tokens), [12_000_000]);
  for (const source of ['tickets', 'commits', 'usage', 'charges'] as const) {
    ...
    if (source === 'usage') {
      ...
      assert.equal(coverage.eligible, coverage.linked + coverage.missing +
        (source === 'tickets' ? 1 : 0),
        `${source} eligible reconciliation`);
      assert.equal(coverage.extracted, coverage.linked + coverage.excluded +
        (source === 'tickets' ? 1 : 0), `${source} extraction reconciliation`);
    }
    assert.equal(bundle[source]!.every(record => record.provenance.source &&
      record.provenance.recordId), true, `${source} provenance`);
  }
...
test('separate USD migration ledger records verification and excludes overlapping hours', () => {
  const ledger = JSON.parse(readFileSync('data/migration.json', 'utf8')) as {
    version: number; currency: string; period: string;
  ...
  assert.deepEqual(ledger.claims.map(claim => [claim.id, claim.amount, claim.verificationStatus]), [
  ...
  const verifiedCosts = ledger.claims.filter(claim => claim.category === 'cost' &&
    claim.verificationStatus === 'verified').reduce((total, claim) => total + claim.amount, 0);
  const verifiedAvoidedCost = ledger.claims.filter(claim => claim.category === 'avoided-cost' &&
    claim.verificationStatus === 'verified').reduce((total, claim) => total + claim.amount, 0);
