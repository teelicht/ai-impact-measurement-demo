...
function commit(repo: string, name: string, file: string, message: string, date: string,
  committedAt = date, email = 'private@example.org'): string {
  ...
  execFileSync('git', ['-C', repo, 'commit', '-m', message], {
    env: { ...process.env, GIT_AUTHOR_NAME: name, GIT_COMMITTER_NAME: name,
      ...
      GIT_AUTHOR_DATE: date, GIT_COMMITTER_DATE: committedAt },
  });
...
function withRepo(run: (repo: string) => Promise<void> | void): Promise<void> {
  ...
  return Promise.resolve().then(() => run(repo)).finally(() => rmSync(repo, { recursive: true, force: true }));
}

const since = '2026-03-01';
const until = '2026-03-31';

test('local Git returns bounded eligible history, paths, links and immutable revision', () => withRepo(repo => {
  ...
  assert.deepEqual(result.commits.map(item => item.hash), [second, first]);
  assert.deepEqual(result.commits.map(item => item.parents), [[first], []]);
  assert.deepEqual(result.commits.map(item => item.paths), [['feature.ts'], ['feature.test.ts']]);
  assert.deepEqual(result.commits.map(item => item.ticketIds), [[], ['API-101']]);
...
test('local Git keeps only pseudonymous distinct authors and classifies test/source additions', () => withRepo(repo => {
  const first = commit(repo, 'One', 'feature.ts', 'Source', '2026-03-05T12:00:00Z',
    '2026-03-05T12:00:00Z', 'one@example.org');
  const second = commit(repo, 'Two', 'feature.test.ts', 'Test', '2026-03-06T12:00:00Z',
    '2026-03-06T12:00:00Z', 'two@example.org');
...
test('invalid repository and revision fail without falling back to synthetic commits', () => withRepo(repo => {
...
test('Git subprocess failures do not reveal the absolute repository path', () => withRepo(async repo => {
  ...
  writeFileSync(configPath, JSON.stringify({
    version: 1, scope: 'Team', startDate: since, endDate: until, timezone: 'UTC',
    extractedAt: '2026-04-01T00:00:00Z', currency: 'EUR', completenessAttestation: 'Local history.',
    sources: { commits: { kind: 'git', path: repo, revision: 'HEAD' } },
  }));
  rmSync(join(repo, '.git', 'objects', root.slice(0, 2), root.slice(2)));
  const sanitized = (error: unknown) => error instanceof Error &&
    /git.*(history|extraction|command)/i.test(error.message) && !error.message.includes(repo);
  assert.throws(() => loadLocalGit(repo, 'HEAD', since, until, 'UTC'), sanitized);
  await assert.rejects(loadConfigured(configPath), sanitized);
}));

test('shallow history is partial even when its visible commit falls inside the window', () => withRepo(async repo => {
  ...
  assert.deepEqual(result.commits.map(item => item.hash), [tip]);
  assert.equal(result.coverage.status, 'partial');
  assert.match(result.coverage.reason, /shallow.*history.*incomplete/i);
  writeFileSync(join(shallow, 'tickets.json'), JSON.stringify({ version: 1, tickets: [
  ...
  writeFileSync(configPath, JSON.stringify({
    ...
    extractedAt: '2026-04-01T00:00:00Z', currency: 'EUR', completenessAttestation: 'Local history.',
    sources: {
      tickets: { kind: 'file', path: 'tickets.json', coverage: { status: 'available',
        eligible: 1, extracted: 1, linked: 0, excluded: 0, missing: 0, reason: '' } },
...
test('local calendar boundaries, ambiguous links, bot and merge exclusions are counted once', () => withRepo(repo => {
  ...
  execFileSync('git', ['-C', repo, 'merge', '--no-ff', '-q', '-m', 'Merge topic', 'topic'], {
    env: { ...process.env, GIT_AUTHOR_DATE: '2026-03-04T12:00:00Z',
      GIT_COMMITTER_DATE: '2026-03-04T12:00:00Z' },
  });
...
test('filenames with line breaks and zero-commit windows remain structured', () => withRepo(repo => {
...
test('commit time, not an earlier author time, determines the reporting month', () => withRepo(repo => {
  const hash = commit(repo, 'Developer', 'delayed.ts', 'API-200 delayed',
    '2026-02-25T12:00:00Z', '2026-03-01T00:30:00Z');
  const result = loadLocalGit(repo, hash, since, until, 'UTC', 'API-[0-9]+');
  assert.deepEqual(result.commits.map(item => item.hash), [hash]);
...
test('an old-dated descendant does not hide a later-dated ancestor', () => withRepo(repo => {
  ...
  assert.deepEqual(result.commits.map(item => item.hash), [tip, root]);
}));

test('extracted Go and Python unit-test paths contribute to the test-touch metric', () => withRepo(async repo => {
...
test('configured Git selector loads the pinned history with no exposed local repository path', () => withRepo(async repo => {
...
test('mixed ticket file and Git preserve the declared report extraction date', () => withRepo(async repo => {
  const hash = commit(repo, 'Developer', 'feature.ts', 'API-101 change', '2026-03-05T12:00:00Z');
  writeFileSync(join(repo, 'tickets.json'), JSON.stringify({ version: 1, tickets: [
    { id: 'API-101', type: 'Story', createdAt: '2026-03-01T09:00:00Z', statusEvents: [] },
  ] }));
  const configPath = join(repo, 'config.json');
  writeFileSync(configPath, JSON.stringify({
    version: 1, scope: 'Team', startDate: since, endDate: until, timezone: 'UTC',
    extractedAt: '2026-04-01T00:00:00Z', currency: 'EUR', completenessAttestation: 'File and Git extracts.',
    sources: {
      tickets: { kind: 'file', path: 'tickets.json', coverage: { status: 'available',
        eligible: 1, extracted: 1, linked: 0, excluded: 0, missing: 0, reason: '' } },
  ...
  assert.match(bundle.coverage.find(entry => entry.source === 'commits')?.reason ?? '',
    /Git extracted at \d{4}-\d\d-\d\dT\d\d:\d\d:\d\d.*Z/);
  assert.equal(bundle.sources?.commits?.revision, hash);
}));

test('configured Git coverage verifies only keys present in the supplied ticket source', () => withRepo(async repo => {
  ...
  assert.deepEqual(report.monthly[0].commitLinks,
    { linked: 1, unlinked: 2, ambiguous: 1, excluded: 0 });
