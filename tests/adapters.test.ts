...
test("configured report context is optional and never replaced with the synthetic profile", async () => {
	...
	try {
		...
		const context = {
			...
			repositories: ["Service source history"],
...
test("configured ticket file preserves provenance and leaves other sources unavailable", async () => {
	...
	try {
		...
		assert.deepEqual(
			bundle.tickets?.[0].approvals.map((approval) => approval.releaseId),
			["R1", "R2"],
		);
...
test("file selector preserves declared partial coverage for an incomplete export", async () => {
	...
	try {
				...
				sources: {
					tickets: {
						...
						coverage: {
							...
							excluded: 0,
							missing: 1,
							reason: "One eligible ticket was not exported",
						},
					},
				},
			}),
		...
		const bundle = await loadConfigured(configPath);
		assert.deepEqual(
			bundle.coverage.find((entry) => entry.source === "tickets"),
			{
				source: "tickets",
				status: "partial",
				eligible: 2,
...
test("file imports require explicit coverage rather than inferring completeness", async () => {
	...
	try {
		...
		const configPath = join(directory, "config.json");
		await writeFile(configPath, JSON.stringify({ ...config, sources: { tickets: { kind: "file", path: "tickets.json" } } }));
...
test("file coverage must match exported records and reconcile eligible with missing", async () => {
	...
	try {
		...
		const configPath = join(directory, "config.json");
		const coverage = { status: "partial", eligible: 2, extracted: 0, linked: 0, excluded: 0, missing: 2, reason: "One missing ticket" };
		await writeFile(
			configPath,
			JSON.stringify({
				...config,
			...
			JSON.stringify({
				...
				sources: {
					tickets: {
						kind: "file",
						path: "tickets.json",
						coverage: { ...coverage, extracted: 1, missing: 0 },
					},
				},
			}),
		);
			...
			JSON.stringify({
				...
				sources: {
					tickets: {
						kind: "file",
						path: "tickets.json",
						coverage: { ...coverage, status: "available", extracted: 1, missing: 1 },
					},
				},
			}),
		);
...
test("module and file sources reject incomplete exports declared as available", async () => {
	...
	try {
      ...
      return { tickets: [{ id: 'API-101', type: 'Story', status: 'open',
		...
		const coverage = { status: "available", eligible: 2, extracted: 1, linked: 0, excluded: 0, missing: 1, reason: "" };
...
test("missing or malformed configured files reject with the source path", async () => {
	...
	try {
		...
		await writeFile(
			...
			JSON.stringify({
				version: 1,
				tickets: [{ id: "API-101", type: "Story", createdAt: "2026-03-01T09:00:00Z", statusEvents: [{ at: "2026-03-20T09:00:00Z", status: "approved" }] }],
			}),
		);
...
test("direct ticket import rejects invalid timestamps with the ticket ID", async () => {
	...
	try {
		...
		await writeFile(path, JSON.stringify({ version: 1, tickets: [{ id: "API-101", type: "Story", createdAt: "not-a-date", statusEvents: [] }] }));
...
test("ticket open history may precede start but approval must follow start", async () => {
	...
	try {
		...
		await writeFile(
			ticketPath,
			JSON.stringify({ version: 1, tickets: [{ ...ticket, statusEvents: [ticket.statusEvents[0], { at: "2026-03-09T09:00:00Z", status: "approved", releaseId: "R1" }] }] }),
		);
...
test("trusted module receives config and returns validated partial coverage without synthetic fill", async () => {
	...
	try {
      ...
      return { tickets: [{ id: 'API-900', type: 'Story', status: 'open',
		...
		const configPath = join(directory, "config.json");
		await writeFile(configPath, JSON.stringify({ ...config, sources: { tickets: { kind: "module", path: "adapter.mjs" } } }));
		const bundle = await loadConfigured(configPath);
...
test("structured monthly token totals import without event details and reject mixed records", async () => {
	...
	try {
		...
		const bundle = await loadConfigured(configPath);
		assert.deepEqual(bundle.usage, [{ ...record, provenance: { source: "file:usage", recordId: "march-total" } }]);
		await writeFile(
			join(directory, "usage.json"),
			JSON.stringify({
				version: 1,
				usage: [record, { id: "event", at: "2026-03-18T10:00:00Z", model: "model-a", inputTokens: 100, cachedInputTokens: 0, outputTokens: 20, attempt: 1, outcome: "success" }],
			}),
		);
		await writeFile(configPath, JSON.stringify({ ...config, sources: { usage: { ...source, coverage: { ...source.coverage, eligible: 2, extracted: 2 } } } }));
		await assert.rejects(loadConfigured(configPath), /mixed usage modes/i);
		await writeFile(join(directory, "usage.json"), JSON.stringify({ version: 1, usage: [{ ...record, model: "invented-model", ticketId: "API-101" }] }));
		await writeFile(configPath, JSON.stringify({ ...config, sources: { usage: source } }));
...
test("module records are subject to the same record validation as file imports", async () => {
	...
	try {
      ...
      return { usage: [{ id: 'bad-event', at: '2026-03-18T10:00:00Z', model: 'example-model',
		...
		const configPath = join(directory, "config.json");
		await writeFile(configPath, JSON.stringify({ ...config, sources: { usage: { kind: "module", path: "bad.mjs" } } }));
...
test("custom module must disclose source coverage", async () => {
	...
	try {
		await writeFile(
			join(directory, "missing-coverage.mjs"),
			`export async function loadSource() {
      return { tickets: [{ id: 'API-901', type: 'Story', status: 'open',
		...
		const configPath = join(directory, "config.json");
		await writeFile(configPath, JSON.stringify({ ...config, sources: { tickets: { kind: "module", path: "missing-coverage.mjs" } } }));
...
test("module coverage extracted must match every returned source array", async () => {
	...
	try {
        ...
        return ${JSON.stringify({
					...
					coverage: [{ source, status: "partial", eligible: 1, extracted: 0, linked: 0, excluded: 0, missing: 1, reason: "Incomplete export" }],
				})};
      }`,
			);
			await writeFile(configPath, JSON.stringify({ ...config, sources: { [source]: { kind: "module", path: moduleName } } }));
...
test("Git selector rejects a missing repository instead of borrowing synthetic commits", async () => {
	...
	try {
		const configPath = join(directory, "config.json");
		await writeFile(configPath, JSON.stringify({ ...config, sources: { commits: { kind: "git", path: "repo", revision: "HEAD" } } }));
