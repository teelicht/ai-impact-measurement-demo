...
const withoutUsage = (source: SourceBundle): SourceBundle => ({
	...
	coverage: source.coverage.map((entry) =>
		entry.source === "usage"
			? { source: "usage", status: "unavailable", eligible: 0, extracted: 0, linked: 0, excluded: 0, missing: 0, reason: "No usage records in this window fixture." }
			: entry,
	),
});
...
test("configured windows show actual periods, row count and no invented AI adoption event", () => {
	...
	const html = renderHtml(buildReport({ ...withoutUsage(source), sourceKind: "configured", endDate: "2026-10-31", completenessAttestation: "Configured full-month window." }));
...
test("a one-month report marks comparisons unavailable without fictitious zeroes", () => {
	...
	const html = renderHtml(
		buildReport({ ...withoutUsage(source), context: undefined, sourceKind: "configured", endDate: "2025-09-30", completenessAttestation: "Configured full-month window." }),
	);
...
test("thirteen months with one approval per month do not claim increased accepted work", () => {
	...
	const months = Array.from({ length: 13 }, (_, index) => new Date(Date.UTC(2025, 8 + index, 1)).toISOString().slice(0, 7));
	const bundle = {
		...withoutUsage(source),
		sourceKind: "configured" as const,
		endDate: "2026-09-30",
		tickets: months.flatMap((month, index) => [
			{ ...firstTicket, id: `rate-${index}`, createdAt: `${month}-02T09:00:00Z`, approvals: [{ at: `${month}-20T10:00:00Z`, releaseId: `release-${index}` }] },
			{ ...firstTicket, id: `bug-${index}`, type: "Bug", status: "unknown" as const, production: true, createdAt: `${month}-11T09:00:00Z`, approvals: [] },
		]),
		commits: months.map((month, index) => ({ ...firstCommit, hash: `rate-commit-${index}`, at: `${month}-12T12:00:00Z` })),
		charges: months.map((month, index) => ({ ...firstCharge, id: `rate-charge-${index}`, billId: `rate-bill-${index}`, month, amount: 100, billTotal: 100 })),
	};
	const view = buildReport(bundle);
	assert.deepEqual(
		view.operational.periods.map((period) => period.approvedParents),
		[6, 7],
	);
	...
	const partial = buildReport({
		...bundle,
		coverage: bundle.coverage.map((entry) => (entry.source === "tickets" ? { ...entry, status: "partial" as const, reason: "Tickets incomplete." } : entry)),
	});
	...
	const partialSpend = buildReport({
		...bundle,
		coverage: bundle.coverage.map((entry) => (entry.source === "charges" ? { ...entry, status: "partial" as const, reason: "Billing incomplete." } : entry)),
	});
...
test("overview totals qualify partial ticket and Git coverage and context text is inert", () => {
	...
	source.coverage = source.coverage.map((entry) =>
		entry.source === "tickets" || entry.source === "commits" ? { ...entry, status: "partial", reason: "Incomplete export." } : entry,
	);
...
test("overview distinguishes incomplete totals from known zero and unavailable", () => {
	...
	const partial = renderHtml(
		buildReport({
			...
			commits: [],
			coverage: source.coverage.map((entry) =>
				entry.source === "tickets" || entry.source === "commits" ? { ...entry, status: "partial" as const, reason: "Import incomplete." } : entry,
			),
		}),
	);
	...
	const absent = renderHtml(
		buildReport({
			...source,
			commits: undefined,
			coverage: source.coverage.map((entry) => (entry.source === "commits" ? { ...entry, status: "unavailable" as const, reason: "Missing Git export." } : entry)),
		}),
	);
...
test("model usage table separates input, output, cached input and failed attempts", () => {
	...
	source.coverage = source.coverage.map((entry) =>
		entry.source === "usage" ? { ...entry, status: "partial", extracted: 1, eligible: 12, missing: 11, reason: "Only one observed event in this fixture." } : entry,
	);
	const html = renderHtml(buildReport(source));
	...
	assert.match(
		tokenCard,
		/<details><summary>Input \+ output tokens data table<\/summary>[\s\S]*<h3>Usage composition<\/h3>[\s\S]*<caption>Model breakdown across reporting windows<\/caption>/,
	);
});
...
test("monthly totals show no invented model, token-category, retry or task detail", () => {
	...
	assert.match(
		tokenCard,
		/<details><summary>Input \+ output tokens data table<\/summary>[\s\S]*<h3>Usage composition<\/h3>[\s\S]*Model, input\/output\/cache, retry and work-item breakdown unavailable/,
	);
	assert.equal((utilization.match(/<h3>Usage composition<\/h3>/g) ?? []).length, 1);
	const absent = loadSynthetic();
	absent.usage = undefined;
	absent.coverage = absent.coverage.map((entry) =>
		entry.source === "usage" ? { ...entry, status: "unavailable", eligible: 0, extracted: 0, linked: 0, reason: "No usage export." } : entry,
	);
...
test("a real All issues type stays separate from the generated 22-issue total", () => {
	...
	const firstTicket = source.tickets?.[0];
	assert.ok(firstTicket);
	const tickets = Array.from({ length: 21 }, (_, index) => ({ ...firstTicket, id: `story-${index}` }));
	const html = renderHtml(
		buildReport({
			...withoutUsage(source),
			sourceKind: "configured",
			endDate: "2025-09-30",
			tickets: [...tickets, { ...firstTicket, id: "real-all-issues", type: "All issues", status: "unknown", approvals: [] }],
		}),
	);
...
test("a real All created issues type and prototype-like types retain distinct chart totals", () => {
	...
	const firstTicket = source.tickets?.[0];
	assert.ok(firstTicket);
	const tickets = ["__proto__", "constructor", "toString", "All created issues"]
		.map((type, index) => ({
			...firstTicket,
			id: `type-${index}`,
			type,
		...
		.concat([{ ...firstTicket, id: "proto-next-month", type: "__proto__", status: "unknown" as const, approvals: [], createdAt: "2025-10-02T12:00:00Z" }]);
	...
	assert.deepEqual(data.operational.monthly[0].issueTypes, JSON.parse('{"__proto__":1,"constructor":1,"toString":1,"All created issues":1}'));
		...
		buildReport({
			...
			commits: [
				...
				},
				{
					...first,
					hash: "second",
					paths: ["tests/a.test.ts", "src/api.ts"],
					changeSize: 120,
					linesAdded: 40,
					linesDeleted: 80,
					testLinesAdded: 10,
...
test("missing sources remain unavailable rather than turning into favorable zeroes", () => {
	...
	source.coverage = source.coverage.map((entry) =>
		entry.source === "usage" || entry.source === "charges" ? { ...entry, status: "unavailable", reason: "Export not supplied" } : entry,
	);
	const html = renderHtml(buildReport(source));
...
test("prototype-like model labels and hostile coverage status do not corrupt markup", () => {
	const source = loadSynthetic();
	source.usage = [
		{ ...detailedEvent, model: "__proto__" },
		{ ...detailedEvent, id: "detailed-2", provenance: { source: "test", recordId: "detailed-2" }, model: "constructor" },
	];
